...
import { IDEA_PHOTO_MAX_LENGTH, isSafeImageDataUrl } from "./avatar.js";
import { isValidTimeZone } from "./booking.js";
import { normalizeRepeat, normalizeRsvp, normalizeTimeVotes } from "./hangout.js";
...
export const LIMITS = {
  ...
  text: 120,
  description: 280,
  // 256 KB of plans and schedules, plus room for about ten full-size idea
  // photos (IDEA_PHOTO_MAX_LENGTH each; real ones are usually much smaller).
  stateBytes: 1536 * 1024,
};
...
export function busyBlocksFor(member, date) {
  ...
  if (coversDate(member.coverage, date)) {
    ...
    return (member.busy || [])
      .map((block) => ({ start: toTime(block.start), end: toTime(block.end), title: block.title, location: block.location, source: block.source }))
...
export function replaceBusyRange(existing, blocks, { source, from, to }) {
  ...
  const added = (blocks || [])
    .map((block) => ({
      ...
      // A place only ever travels with its event's name (CLAUDE.md).
      ...(block.title
        ? { title: String(block.title).slice(0, LIMITS.text), ...(block.location ? { location: String(block.location).slice(0, LIMITS.text) } : {}) }
        : {}),
...
function normalizeBusy(list) {
  ...
  return list
    .map((block) => {
      ...
      // A place only travels with a name: busy-only blocks carry neither.
      const location = title ? text(block?.location) : "";
      return { start: start.toISOString(), end: end.toISOString(), ...(title ? { title } : {}), ...(location ? { location } : {}), source };
    })
    .filter(Boolean)
    .sort((a, b) => new Date(a.start) - new Date(b.start))
    .slice(0, LIMITS.busyPerMember);
...
function normalizePlan(plan, memberIds) {
  ...
  return {
    ...
    ...(plan.chosen && plan.chosenEnd && new Date(plan.chosenEnd) > new Date(plan.chosen)
      ...
      : {}),
    ...(normalizeRepeat(plan.repeat) !== "none" ? { repeat: normalizeRepeat(plan.repeat) } : {}),
    ...(isValidTimeZone(plan.timeZone) ? { timeZone: plan.timeZone } : {}),
    ...(Object.keys(normalizeTimeVotes(plan.timeVotes, memberIds)).length ? { timeVotes: normalizeTimeVotes(plan.timeVotes, memberIds) } : {}),
    ...(normalizeRsvp(plan.rsvp, memberIds) ? { rsvp: normalizeRsvp(plan.rsvp, memberIds) } : {}),
...
export function normalizeWorkspaceState(input) {
  ...
  return {
    ...
    plan: normalizePlan(raw.plan, memberIds),
