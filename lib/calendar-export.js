...
export function buildPlanIcs(plan, { slug, url, now = new Date() } = {}) {
  ...
  const description = [`Planned with ${plan.audience || "your group"} on Gatherly.`, url ? `Group: ${url}` : ""]
    .filter(Boolean)
    .join("\n");
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Gatherly//Hangout planner//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    "BEGIN:VEVENT",
...
export function googleCalendarUrl(plan, { url } = {}) {
  ...
  const params = new URLSearchParams({
    ...
    details: [`Planned with ${plan.audience || "your group"} on Gatherly.`, url ? `Group: ${url}` : ""].filter(Boolean).join("\n"),
  });
