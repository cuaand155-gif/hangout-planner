...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  picked: [],
  useCalendars: true, // block times the owner is busy in their calendars
});
...
/**
 * The owner's calendar events as the bare busy ranges a booking page stores:
 * start and end only, never names, overlapping ranges merged. Only events
 * that touch the booking window are kept, unclipped, so the result stays the
 * same from one minute to the next and an unchanged calendar is not re-saved.
 * All-day events do not block time, matching calendar links. Private events
 * are included on purpose: they stay closed to bookings.
 */
export function busyForBooking(events, { now = new Date(), windowDays = DEFAULT_SETTINGS.windowDays } = {}) {
  const from = new Date(now).getTime();
  const to = from + (clampInt(windowDays, 1, 60, DEFAULT_SETTINGS.windowDays) + 1) * DAY;
  const ranges = (events || [])
    .filter((event) => event && !event.allDay)
    .map((event) => [new Date(event.start).getTime(), new Date(event.end).getTime()])
    .filter(([start, end]) => Number.isFinite(start) && Number.isFinite(end) && end > start && end > from && start < to)
    .sort((a, b) => a[0] - b[0]);
  const merged = [];
  ...
  return merged.slice(0, MAX_BUSY).map(([start, end]) => ({ start: new Date(start).toISOString(), end: new Date(end).toISOString() }));
}

/**
 * Whether two lists of busy ranges cover the same times. Compares instants,
 * not JSON text: the database hands jsonb objects back with their keys
 * reordered, so a text comparison would never match and re-save every time.
 */
export function sameBusy(a, b) {
  const key = (list) => (Array.isArray(list) ? list : []).map((range) => `${new Date(range?.start).getTime()}-${new Date(range?.end).getTime()}`).join(",");
  return key(a) === key(b);
}
...
/** The share link, the subscribable feed and a one-click Google "add by URL" link. */
export function bookingLinks(origin, handle, feedToken) {
  const base = String(origin || "").replace(/\/+$/, "");
  const host = base.replace(/^https?:\/\//, "");
  const feed = feedToken ? `${host}/api/book?feed=${encodeURIComponent(feedToken)}` : "";
  return {
    page: `${base}/book/${encodeURIComponent(handle)}`,
    webcal: feed ? `webcal://${feed}` : "",
    https: feed ? `https://${feed}` : "",
    google: feed ? `https://calendar.google.com/calendar/r?cid=${encodeURIComponent(`webcal://${feed}`)}` : "",
  };
}

/** A default handle from a name: "Alexi Cua" -> "alexi-cua-7fq2x". */
export function suggestHandle(name, random = Math.random) {
  const alphabet = "abcdefghjkmnpqrstuvwxyz23456789";
  const base = String(name || "me")
    .toLowerCase()
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9]+/g, "-")
    ...
    .slice(0, 30) || "me";
  let suffix = "";
  for (let index = 0; index < 5; index += 1) suffix += alphabet[Math.floor(random() * alphabet.length) % alphabet.length];
  return normalizeHandle(`${base}-${suffix}`) || `me-${suffix}`;
}
