...
function unescapeText(value) {
  // One pass, so an escaped backslash before an "n" isn't read as a newline.
  return String(value)
    .replace(/\\([\\;,nN])/g, (_, character) => (character === "n" || character === "N" ? " " : character))
    .trim();
}
  ...
  for (const event of parsedEvents) {
    ...
    for (const start of expand(event, windowStart, windowEnd)) {
      ...
      blocks.push({
        ...
        ...(includeTitles && event.location ? { location: event.location.slice(0, 120) } : {}),
      });
    }
  }
