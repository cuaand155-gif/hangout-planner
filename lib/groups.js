...
/**
 * Keeps a listed group's name in step after it is renamed. Only an entry that
 * is already there changes, and its place in the list stays put. Returns the
 * same list when nothing changed.
 */
export function renameGroup(list, slug, name) {
  const entries = Array.isArray(list) ? list : [];
  if (!name || !entries.some((entry) => valid(entry) && entry.slug === slug && entry.name !== name)) return entries;
  return entries.map((entry) => (valid(entry) && entry.slug === slug ? { ...entry, name } : entry));
}
